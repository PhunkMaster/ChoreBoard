"""
DEBUG SCRIPT: Verify Exclude From Auto-Assignment Feature

Purpose: Verifies that the exclude_from_auto_assignment user field works correctly
         and that excluded users are not auto-assigned chores.

Usage: python debug/verify_exclude_auto_assignment.py

This script:
1. Checks that the exclude_from_auto_assignment field exists on User model
2. Displays current users and their auto-assignment status
3. Tests AssignmentService._get_eligible_users() filtering
4. Confirms manual assignment still works for excluded users

Useful for: Debugging user auto-assignment issues
"""
import os
