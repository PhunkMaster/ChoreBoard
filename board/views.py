...
def main_board(request):
    ...
    pool_chores = (
        ...
        .prefetch_related("completion__completed_by", "completion__shares__user")
        .order_by("due_at")
    )
    ...
    active_arcade_sessions = (
        ArcadeSession.objects.filter(status=ArcadeSession.STATUS_ACTIVE)
...
def user_board(request, username):
    ...
    assigned_chores = (
        ChoreInstance.objects.filter(
            assigned_to=user, status__in=[ChoreInstance.ASSIGNED, ChoreInstance.POOL]
        )
        .filter(
            Q(chore__is_active=True)  # Active chores
            | Q(  # OR open instances of inactive chores
    ...
    users = User.objects.filter(
        is_active=True, can_be_assigned=True, eligible_for_points=True
    ).order_by("first_name", "username")

    # Check for active arcade session for THIS user only (kiosk-mode compatible)
    from chores.models import ArcadeSession

    active_arcade_session = (
...
def user_board_minimal(request, username):
    ...
    assigned_chores = (
        ...
        .filter(
            ...
            | Q(due_at__lt=today_start)  # Due today OR past due
        )
        .select_related("chore")
        .prefetch_related("completion__completed_by", "completion__shares__user")
...
def assigned_minimal(request):
    ...
    assigned_chores = (
        ChoreInstance.objects.filter(status=ChoreInstance.ASSIGNED)
        .filter(
            Q(chore__is_active=True)  # Active chores
            | Q(  # OR open instances of inactive chores
                chore__is_active=False,
        ...
        .exclude(status=ChoreInstance.SKIPPED)
        .select_related("chore", "assigned_to")
        .prefetch_related("completion__completed_by", "completion__shares__user")
        .order_by("due_at")
    )
...
def users_minimal(request):
    ...
    for user in users:
        users_with_counts.append(
            {"user": user, "chore_count": chore_counts.get(user.id, 0)}
        )
...
def quick_add_task(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "board/quick_add_task.html", {"users": users, "chores": chores}
        )
        ...
        try:
            ...
            if not (
                ...
                try:
                    ...
                    if points < 0 or points > 999.99:
                        return JsonResponse(
                            {"error": "Points must be between 0 and 999.99"}, status=400
                        )
                ...
                if assignment_type == "assigned" and assigned_to_id:
                    try:
                        assigned_user = User.objects.get(
                            id=assigned_to_id, is_active=True
                        )
                    except User.DoesNotExist:
                        return JsonResponse(
                            {"error": "Invalid user selected for assignment"},
                            status=400,
                        )
                elif assignment_type == "assigned":
                    return JsonResponse(
                        {"error": "Please select a user for assignment"}, status=400
                    )
            ...
            if depends_on_id:
                try:
                    ...
                    return JsonResponse(
                        {"error": "Invalid parent chore selected"}, status=400
                    )
            ...
            if due_date_str and not depends_on_id:
                try:
                    one_time_due_date = datetime.strptime(
                        due_date_str, "%Y-%m-%d"
                    ).date()
                except ValueError:
                    return JsonResponse(
                        {"error": "Invalid due date format"}, status=400
                    )
            ...
            with transaction.atomic():
                ...
                if parent_chore:
                    ...
                    ChoreDependency.objects.create(chore=chore, depends_on=parent_chore)
...
def claim_chore_view(request):
    ...
    try:
        ...
        if not user_id:
            return JsonResponse(
                {"error": "Please select who is claiming this chore"}, status=400
            )
        ...
        with transaction.atomic():
            ...
            if instance.status != ChoreInstance.POOL:
                return JsonResponse(
                    {"error": "This chore is not in the pool"}, status=400
                )
...
def complete_chore_view(request):
    ...
    try:
        ...
        if not user_id:
            return JsonResponse(
                {"error": "Please select who is completing this chore"}, status=400
            )
        ...
        with transaction.atomic():
            ...
            try:
                ...
                if completion.is_undone:
                    ...
                    return JsonResponse(
                        {"error": "Completion record already exists"}, status=400
                    )
            except Completion.DoesNotExist:
                # No existing completion, create new one
                completion = Completion.objects.create(
                    chore_instance=instance, completed_by=user, was_late=was_late
                )
...
def skip_chore_view(request):
    ...
    try:
        ...
        if not user_id:
            return JsonResponse(
                {"error": "Please select who is skipping this chore"}, status=400
            )
        ...
        if not (user.is_staff or user.is_superuser):
            return JsonResponse(
                {"error": "Only administrators can skip chores"}, status=403
            )
...
def reschedule_chore_view(request):
    ...
    try:
        ...
        if not user_id:
            return JsonResponse(
                {"error": "Please select who is rescheduling this chore"}, status=400
            )
        if not new_due_datetime:
            return JsonResponse(
                {"error": "Please provide a new due date and time"}, status=400
            )
        ...
        if not (user.is_staff or user.is_superuser):
            return JsonResponse(
                {"error": "Only administrators can reschedule chores"}, status=403
            )
        ...
        try:
            ...
            if not new_due_at:
                ...
                new_due_at = timezone.datetime.fromisoformat(
                    new_due_datetime.replace("Z", "+00:00")
                )
        ...
        with transaction.atomic():
            ...
            if instance.status == ChoreInstance.COMPLETED:
                return JsonResponse(
                    {"error": "Cannot reschedule a completed chore"}, status=400
                )
            ...
            logger.info(
                f"User {user.username} rescheduled chore {instance.chore.name} from {old_due_at} to {new_due_at}"
            )

            return JsonResponse(
                {
                    "message": f'Chore rescheduled to {new_due_at.strftime("%B %d at %I:%M %p")}'
                }
            )
...
def get_updates(request):
    ...
    try:
        ...
        updates = {"timestamp": now.isoformat(), "changes": []}

        # Get updated chore instances: include chores due today OR overdue from previous days
        updated_instances = (
            ChoreInstance.objects.filter(updated_at__gt=since)
            .filter(
                Q(chore__is_active=True)  # Active chores
                | Q(  # OR open instances of inactive chores
                    chore__is_active=False,
        ...
        for instance in updated_instances:
            change_data = {
                ...
                "data": {
                    ...
                    "assigned_to": (
                        instance.assigned_to.get_display_name()
                        if instance.assigned_to
                        else None
                    ),
                    "assigned_to_id": (
                        instance.assigned_to.id if instance.assigned_to else None
                    ),
                },
            }
        ...
        updated_users = User.objects.filter(
            is_active=True, eligible_for_points=True, updated_at__gt=since
        )
