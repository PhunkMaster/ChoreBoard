...
def start_arcade(request):
    ...
    try:
        ...
        if not instance_id:
            return JsonResponse(
                {"success": False, "message": "Missing instance_id"}, status=400
            )
        ...
        if user_id:
            ...
            return JsonResponse(
                {"success": False, "message": "User must be specified"}, status=400
            )

        success, message, arcade_session = ArcadeService.start_arcade(
            user, chore_instance
        )
...
def stop_arcade(request):
    ...
    try:
        ...
        if not session_id:
            return JsonResponse(
                {"success": False, "message": "Missing session_id"}, status=400
            )
...
def judge_select(request, session_id):
    ...
    available_judges = (
        ...
        .exclude(id=arcade_session.user.id)
...
def approve_submission(request, session_id):
    ...
    try:
        ...
        logger.info(
            f"Session user: {arcade_session.user.username} (ID: {arcade_session.user.id})"
        )
        ...
        success, message, arcade_completion = ArcadeService.approve_arcade(
            arcade_session, judge=judge, notes=notes
        )
...
def arcade_leaderboard(request):
    ...
    chores_with_scores = (
        Chore.objects.filter(high_scores__isnull=False, is_active=True)
        .distinct()
        .order_by("name")
    )
    ...
    for chore in chores_with_scores:
        ...
        if top_scores.exists():
            leaderboard_data.append({"chore": chore, "top_scores": top_scores})
    ...
    if filter_chore:
        try:
            ...
            leaderboard_data = [
                item for item in leaderboard_data if item["chore"].id == chore_obj.id
            ]
            ...
            leaderboard_data = [
                {
                    **item,
                    "top_scores": [
                        score for score in item["top_scores"] if score.user == user_obj
                    ],
                }
...
def user_profile(request, username):
    ...
    personal_bests = (
        ArcadeHighScore.objects.filter(user=user, chore__is_active=True)
        .select_related("chore", "arcade_completion")
        .order_by("time_seconds", "chore__name")
    )

    # Get recent arcade history - only for active chores
    recent_completions = (
        ArcadeCompletion.objects.filter(user=user, chore__is_active=True)
    ...
    recent_denials = (
        ArcadeSession.objects.filter(
            user=user, status=ArcadeSession.STATUS_DENIED, chore__is_active=True
        )
        .select_related("chore")
    ...
    if user.eligible_for_streaks:
        ...
        perfect_week_streak = streak.current_streak
    ...
    context = {
        ...
        "perfect_week_streak": perfect_week_streak,
    }
