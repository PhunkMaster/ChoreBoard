...
def admin_dashboard(request):
    ...
    pool_count = (
        ChoreInstance.objects.filter(status=ChoreInstance.POOL, chore__is_active=True)
        .filter(
    ...
    overdue_count = (
        ...
        .filter(Q(assigned_to__isnull=True) | Q(assigned_to__eligible_for_points=True))
        .filter(
            Q(
                due_at__range=(today_start, today_end)
            )  # Due today but overdue (timezone-aware)
            | Q(due_at__lt=today_start)  # Overdue from previous days
            | Q(
                due_at__gte=far_future
            )  # No due date (can't be overdue but include for consistency)
        )
        .count()
    )
    ...
    recent_completions = (
        Completion.objects.filter(completed_at__gte=now - timedelta(hours=24))
        .select_related("chore_instance__chore", "completed_by")
        .order_by("-completed_at")[:10]
    )

    # Recent actions
    recent_actions = ActionLog.objects.select_related("user").order_by("-created_at")[
        :15
    ]
...
def admin_chores(request):
    ...
    for chore in chores:
        last_instance = (
            ChoreInstance.objects.filter(chore=chore).order_by("-created_at").first()
        )
...
def admin_chore_history(request, chore_id):
    ...
    instances = (
        ChoreInstance.objects.filter(chore=chore)
        .select_related("assigned_to", "completion__completed_by", "skipped_by")
...
def admin_chore_create_instance(request, chore_id):
    ...
    try:
        ...
        if chore.is_undesirable and status == ChoreInstance.POOL:
            ...
            status_msg = (
                f"assigned to {assigned_to.username}" if assigned_to else "in pool"
            )
            return JsonResponse(
                {
                    "success": True,
        ...
        return JsonResponse({"error": f"Error creating instance: {str(e)}"}, status=500)
...
def admin_users_list(request):
    ...
    """
    try:
        users = User.objects.filter(is_active=True, can_be_assigned=True).order_by(
            "first_name", "username"
        )
...
def admin_settings(request):
    ...
    if request.method == "POST":
        try:
            ...
            settings.weekly_reset_undo_hours = request.POST.get(
            ...
            settings.arcade_submission_redirect_seconds = request.POST.get(
            ...
            site_settings.points_label = request.POST.get(
                "points_label", "points"
            ).strip()
            site_settings.points_label_short = request.POST.get(
...
def admin_undo_completions(request):
    ...
    recent_completions = (
        Completion.objects.filter(completed_at__gte=cutoff, is_undone=False)
        .select_related("chore_instance__chore", "completed_by")
    ...
    try:
        ...
        with transaction.atomic():
            completion = get_object_or_404(
                Completion.objects.select_related("chore_instance"), id=completion_id
            )
            ...
            logger.info(
                f"Admin {request.user.username} undid completion {completion.id}"
            )
...
def admin_reset_arcade_times(request):
    ...
    context = {
        "active_page": "reset_arcade",
    }
    return render(request, "board/admin/reset_arcade_confirm.html", context)
...
def admin_backdate_completion_action(request):
    ...
    try:
        ...
        if completion_date > today:
            return JsonResponse(
                {"error": "Completion date must be in the past or today"}, status=400
            )
        ...
        with transaction.atomic():
            ...
            if helper_ids:
                ...
                if instance.chore.is_undesirable:
                    ...
                    helpers_list = list(
                        User.objects.filter(
                            id__in=eligible_ids, eligible_for_points=True
                        )
                    )
                else:
            ...
            if helpers_list:
                ...
                for helper in helpers_list:
                    ...
                    if is_current_week:
                        ...
                        helper.add_points(
                            points_per_person, weekly=False, all_time=True
                        )
                    ...
                    PointsLedger.objects.create(
                        ...
                        balance_after=(
                            helper.weekly_points
                            if is_current_week
                            else snapshot.points_earned
                        ),
                        completion=completion,
...
def admin_chore_get(request, chore_id):
    ...
    try:
        ...
        eligible_user_ids = list(
            ChoreEligibility.objects.filter(chore=chore).values_list(
                "user_id", flat=True
            )
        )
        ...
        data = {
            ...
            "n_days": chore.n_days,
            "every_n_start_date": (
                chore.every_n_start_date.isoformat()
                if chore.every_n_start_date
                else None
            ),
            "cron_expr": chore.cron_expr or "",
            "rrule_json": chore.rrule_json or "",
            "one_time_due_date": (
                chore.one_time_due_date.isoformat() if chore.one_time_due_date else ""
            ),
            "depends_on": depends_on_id,
            "offset_hours": offset_hours,
            "is_active": chore.is_active,
            "assignable_users": [
                {"id": u.id, "name": u.get_display_name()} for u in assignable_users
            ],
        }
...
def admin_chore_create(request):
    ...
    try:
        ...
        if len(name) > 255:
            return JsonResponse(
                {"error": "Chore name cannot exceed 255 characters"}, status=400
            )

        if points < 0 or points > Decimal("999.99"):
            return JsonResponse(
                {"error": "Points must be between 0.00 and 999.99"}, status=400
            )

        if not is_pool and not assigned_to_id:
            return JsonResponse(
                {"error": "Non-pool chores must have an assigned user"}, status=400
            )
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Created chore {chore.id}: {chore.name}, is_undesirable={chore.is_undesirable}"
            )
                ...
                try:
                    ...
                    for user_id in eligible_user_ids:
                        ChoreEligibility.objects.create(
                            chore=chore, user_id=int(user_id)
                        )
                    logger.info(
                        f"Created {len(eligible_user_ids)} ChoreEligibility records for {chore.name}"
                    )
                    ...
                    pool_instances = ChoreInstance.objects.filter(
                        chore=chore, status=ChoreInstance.POOL, assigned_to__isnull=True
                    )

                    for instance in pool_instances:
                        success, message, assigned_user = (
                            AssignmentService.assign_chore(instance)
                        )
                        if success:
                            logger.info(
                                f"✓ Assigned undesirable chore {chore.name} to {assigned_user.username}"
                            )
                        else:
                            logger.warning(
            ...
            logger.info(
                f"Admin {request.user.username} created chore {chore.id}: {chore.name}"
            )
...
def admin_chore_update(request, chore_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            chore.every_n_start_date = (
                every_n_start_date if every_n_start_date else None
            )
            ...
            logger.info(
                f"Admin {request.user.username} updated chore {chore.id}: {chore.name}"
            )
...
def admin_chore_toggle_active(request, chore_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} {status} chore {chore.id}: {chore.name}"
            )
...
def admin_chore_delete(request, chore_id):
    ...
    try:
        ...
        logger.info(
            f"Admin {request.user.username} permanently deleted chore {chore_id}: {chore_name}"
        )

        return JsonResponse({"message": f'Chore "{chore_name}" deleted successfully'})
...
def admin_template_get(request, template_id):
    ...
    try:
        ...
        data = {
            ...
            "every_n_start_date": (
                template.every_n_start_date.isoformat()
                if template.every_n_start_date
                else None
            ),
...
def admin_template_save(request):
    ...
    try:
        ...
        existing_template = ChoreTemplate.objects.filter(
        ...
        if existing_template:
            ...
            existing_template.every_n_start_date = (
                every_n_start_date if every_n_start_date else None
            )
...
def admin_user_create(request):
    ...
    try:
        ...
        if len(username) > 150:
            return JsonResponse(
                {"error": "Username cannot exceed 150 characters"}, status=400
            )
        ...
        if len(password) < 4:
            return JsonResponse(
                {"error": "Password must be at least 4 characters"}, status=400
            )
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} created user {user.id}: {user.username}"
            )
...
def admin_user_update(request, user_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} updated user {user.id}: {user.username}"
            )
...
def admin_user_toggle_active(request, user_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} {status} user {user.id}: {user.username}"
            )
...
def admin_backup_create(request):
    ...
    try:
        ...
        return JsonResponse(
            {"message": "Backup created successfully", "output": output}
        )
...
def admin_backup_create_selective(request):
    ...
    try:
        ...
        output = out.getvalue()
        logger.info(
            f"Admin {request.user.username} created selective backup: {filename}"
        )
...
def admin_backup_delete(request, backup_id):
    ...
    try:
        ...
        ActionLog.objects.create(
            ...
            metadata={"backup_id": backup_id, "filename": filename},
        )
...
def admin_backup_download(request, backup_id):
    ...
    try:
        ...
        logger.info(
            f"Admin {request.user.username} downloading backup {backup.filename}"
        )

        # Return file as download
        response = FileResponse(
            open(backup.file_path, "rb"), as_attachment=True, filename=backup.filename
        )
...
def admin_backup_upload(request):
    ...
    try:
        ...
        if not uploaded_file.name.endswith(".sqlite3"):
            return JsonResponse(
                {"error": "File must be a .sqlite3 database file"}, status=400
            )
        ...
        if uploaded_file.size > max_size:
            return JsonResponse(
                {"error": f"File too large. Maximum size is 500MB"}, status=400
            )
        ...
        try:
            ...
            if temp_path.exists():
                os.remove(temp_path)
            return JsonResponse(
                {"error": f"Invalid SQLite database: {str(e)}"}, status=400
            )
        ...
        backup_type_label = (
            "Full backup" if not is_selective else "Selective backup (clean database)"
        )
        logger.info(
            f"Admin {request.user.username} uploaded backup {filename} ({backup_type_label})"
        )
...
def admin_backup_restore(request):
    ...
    try:
        ...
        logger.info(
            f"Admin {request.user.username} queued restore of {backup.filename}"
        )
...
def admin_force_assign(request):
    ...
    pool_chores = (
        ChoreInstance.objects.filter(status=ChoreInstance.POOL)
        .select_related("chore")
        .order_by("due_at")
    )
...
def admin_force_assign_action(request, instance_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} force-assigned chore {instance.id} to {user.username}"
            )
...
def admin_unassign_action(request, instance_id):
    ...
    try:
        ...
        if (
            ...
            return JsonResponse(
                {
                    "error": "Can only unassign force-assigned, manually assigned, fixed, or undesirable chores"
                },
                status=400,
            )
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} unassigned chore {instance.id} from {old_user.username}"
            )
...
def admin_reassign_action(request, instance_id):
    ...
    try:
        ...
        if (
            ...
            return JsonResponse(
                {
                    "error": "Can only reassign force-assigned, manually assigned, fixed, or undesirable chores"
                },
                status=400,
            )
        ...
        if instance.assigned_to == new_user:
            return JsonResponse(
                {"error": "Chore is already assigned to this user"}, status=400
            )
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} reassigned chore {instance.id} from {old_user.username} to {new_user.username}"
            )
...
def admin_streak_increment(request, user_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} incremented streak for {user.username}"
            )
...
def admin_streak_reset(request, user_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Admin {request.user.username} reset streak for {user.username}"
            )
...
def admin_skip_chore(request, instance_id):
    ...
    try:
        ...
        if success:
            logger.info(
                f"Admin {request.user.username} skipped chore instance {instance_id}"
            )
            return JsonResponse(
                {
                    "message": message,
...
def admin_unskip_chore(request, instance_id):
    ...
    try:
        success, message, instance = SkipService.unskip_chore(
            instance_id=instance_id, user=request.user
        )

        if success:
            logger.info(
                f"Admin {request.user.username} unskipped chore instance {instance_id}"
            )
            return JsonResponse(
                {
                    "message": message,
...
def admin_reschedule_chore(request, chore_id):
    ...
    try:
        ...
        if success:
            logger.info(
                f"Admin {request.user.username} rescheduled chore {chore_id} to {new_date}"
            )
            return JsonResponse(
                {
                    "message": message,
                    ...
                    "chore_name": chore.name,
                    "rescheduled_date": (
                        chore.rescheduled_date.isoformat()
...
def admin_clear_reschedule(request, chore_id):
    ...
    try:
        success, message, chore = RescheduleService.clear_reschedule(
            chore_id=chore_id, user=request.user
        )

        if success:
            logger.info(
                f"Admin {request.user.username} cleared reschedule for chore {chore_id}"
            )
            return JsonResponse(
                {"message": message, "chore_id": chore.id, "chore_name": chore.name}
            )
        else:
            logger.warning(
                f"Failed to clear reschedule for chore {chore_id}: {message}"
            )
            return JsonResponse({"error": message}, status=400)
...
def admin_skip_chores(request):
    ...
    active_chores = (
        ...
        .select_related("chore", "assigned_to")
        .order_by("due_at", "status")
    )

    # Get recently skipped chores (within undo window)
    skipped_chores_recent = (
        ChoreInstance.objects.filter(
            status=ChoreInstance.SKIPPED, skipped_at__gte=undo_cutoff
        )
        .select_related("chore", "assigned_to", "skipped_by")
        .order_by("-skipped_at")
    )
    ...
    skipped_chores_old = (
        ChoreInstance.objects.filter(
            status=ChoreInstance.SKIPPED, skipped_at__lt=undo_cutoff
        )
        .select_related("chore", "assigned_to", "skipped_by")
        .order_by("-skipped_at")[:20]
    )
...
def admin_reschedule_chores(request):
    ...
    active_chores = (
        ...
        .prefetch_related("eligible_users")
        .order_by("name")
    )
...
def admin_adjust_points(request):
    ...
    recent_adjustments = (
        PointsLedger.objects.filter(transaction_type=PointsLedger.TYPE_ADMIN_ADJUSTMENT)
...
def admin_adjust_points_submit(request):
    ...
    try:
        ...
        if abs(points) > Decimal("999.99"):
            return JsonResponse(
                {"error": "Points adjustment cannot exceed ±999.99"}, status=400
            )
        ...
        if user.id == request.user.id:
            return JsonResponse(
                {"error": "You cannot adjust your own points"}, status=403
            )
...
def admin_pending_spawns(request):
    ...
    try:
        ...
        todays_completions = Completion.objects.filter(
            completed_at__date=today, is_undone=False
        ).select_related("chore_instance__chore", "completed_by")
            ...
            for dep in dependencies:
                ...
                if spawn_time > now:
                    ...
                    existing_instance = ChoreInstance.objects.filter(
                        chore=child_chore, created_at__gte=completion.completed_at
                    ).first()
                    ...
                    if not existing_instance:
                        ...
                        if hours_remaining >= 1:
                            relative_time = (
                                f"in {int(hours_remaining)}h {int(minutes_remaining)}m"
                            )
                        else:
                        ...
                        pending_spawns.append(
                            {
                                ...
                                "parent_completed_at": local_completed_at.strftime(
                                    "%I:%M %p"
                                ),
                                "completed_by": (
                                    completion.completed_by.get_display_name()
                                    if completion.completed_by
                                    else "Unknown"
                                ),
                                "spawn_time_absolute": local_spawn_time.strftime(
                                    "%I:%M %p"
                                ),
...
def admin_force_spawn(request):
    ...
    try:
        ...
        dependency = ChoreDependency.objects.filter(
            chore=child_chore, depends_on=completion.chore_instance.chore
        ).first()
...
def save_user_preferences(request):
    ...
    try:
        ...
        logger.info(
            f"User {request.user.username} updated quick actions: {quick_actions}"
        )
...
def admin_midnight_evaluation(request):
    ...
    today_eval = (
        EvaluationLog.objects.filter(
            started_at__gte=today_start, started_at__lt=today_end
        )
        .order_by("-started_at")
        .first()
    )
...
def admin_midnight_evaluation_run(request):
    ...
    try:
        ...
        if os.getenv("CELERY_BROKER_URL"):
            ...
            messages.success(
                request, "Midnight evaluation task has been queued in Celery!"
            )
        else:
            midnight_evaluation()
            messages.success(
                request, "Midnight evaluation completed successfully (synchronous)!"
            )
...
def admin_midnight_evaluation_check(request):
    ...
    try:
        ...
        if not eval_exists:
            logger.warning(
                f"Watchdog check by {request.user.username}: No evaluation today"
            )
            logger.info(
                f"Running missed midnight evaluation (triggered by {request.user.username})"
            )
            ...
            if os.getenv("CELERY_BROKER_URL"):
                ...
                messages.success(
                    request,
                    "Watchdog check: Missed evaluation detected. Task queued in Celery!",
                )
            else:
                midnight_evaluation()
                messages.success(
                    request,
                    "Watchdog check: Missed evaluation detected and executed (synchronous)!",
                )
        else:
            messages.info(
                request,
                "Watchdog check: Midnight evaluation already ran today. No action needed.",
            )
